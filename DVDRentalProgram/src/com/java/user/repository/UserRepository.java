...
public class UserRepository {
	...
	public void addUser(User user) {
		...
		String insertSql = "INSERT INTO users (user_number, user_name, phone_number)"
				+ "VALUES(users_seq.nextval,?,?)";
		/*
		 * try with Resources
		 * 자바 1.8 이후에 사용할 수 있는 문법.
		 * try문에 자원 객체를 전달하면, try가 끝날 경우
		 * 자동으로 자원을 종료해주는 기능. (finally에서 따로 close() 안해도됨)
		 * 자원을 종료하려는 객체가 AutoCloseable 인터페이스의 구현체여야 한다.
		 */
		try (Connection conn = connection.getConnection();
			...
			if(pstmt.executeUpdate()==1) {
				...
				System.out.println("회원가입에 실패했습니다. 관리자에게 문의하세요.");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		public void deleteUser(int delUserNum) {
			String deleteSql = "DELETE FROM users WHERE user_number = ?";
			try(Connection conn = connection.getConnection();
